...
def fetch_and_process_streaming(bucket_name, base_prefix, num_fetch_threads=10, num_processes=2):
    ...
    with ThreadPoolExecutor(max_workers=num_fetch_threads) as fetch_executor, \
        ...
        for future in tqdm(concurrent.futures.as_completed(future_to_key), total=len(future_to_key), desc="Streaming"):
            key = future_to_key[future]
            try:
                content = future.result()
                if content:
                    process_executor.submit(process_file, key, content)
            except Exception as e:
                print(f"Error while fetching or processing {key}: {e}")
...
def main():
    ...
    base_prefix = "AWSLogs/375872583153/WAFLogs/cloudfront/honasa-mmrth-waf-prod/2025/04/06/18/"
