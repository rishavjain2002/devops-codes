...
from credentials import region_name, aws_access_key_id, aws_secret_access_key, BATCH_SIZE, BUCKET_NAME, queue_url_omni 
        ...
        if file_content:
            ...
            if len(result_list) > 0:
                ...
                for i in range(0, len(result_list), BATCH_SIZE):
                    ...
                    path_to_file = os.path.join(server_path, "../data/omni-elb-logs")
                    print(path_to_file)
                    save_data_as_parquet(batch, output_folder=path_to_file)
                    upload_to_s3(path_to_file, "omni-elb-logs")
